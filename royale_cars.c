/*Alunos:                     turma: IINF11A
Gideão Ximenes de Melo Sampaio
Raimundo Nathanyel dos Santos Viana
Sophia Costa Seixas
*/
//Bibliotecas
#include <stdio.h>
...
// --------------------Inserir bordas abaixo das frases-----------------------
...
int verifica_ID(char id[],int tam,int tam1){
	...
	//verifica se há um codigo existente
	while(fread(&c,sizeof(c),1,ark)){
		if(strcmp(id,c.codigo)==0){
			i++;
		}
...
void Balanco_Acervo(void){
	...
	//conta quantos clientes tem no arquivo
	rewind(arkCliente);
